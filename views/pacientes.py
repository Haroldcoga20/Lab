...
class PacientesView(ft.Column):
    def __init__(self, page):
        super().__init__()
        self.page_ref = page # Store page reference just in case, though self.page works when mounted
        self.expand = True
                ...
                self.table.rows.append(
                    ft.DataRow(cells=[
                        ...
                        ft.DataCell(ft.Row([
                            ...
                            ft.IconButton(ft.Icons.DELETE, icon_color=ft.Colors.RED, on_click=lambda e, pid=p.id: self.delete_paciente_click(pid))
                        ])),
                    ])
                )
    ...
    def delete_paciente_click(self, paciente_id):
        # Could show confirmation dialog, but requirements asked for "Action Delete" directly.
        # Let's add simple delete call.
        try:
            db.delete_paciente(paciente_id)
            self.page.open(ft.SnackBar(ft.Text("Paciente eliminado"), bgcolor=ft.Colors.GREEN))
            self.load_data()
        except Exception as e:
            self.page.open(ft.SnackBar(ft.Text(f"Error al eliminar: {e}"), bgcolor=ft.Colors.RED))
