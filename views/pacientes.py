...
class PacientesView(ft.Column):
    ...
    def confirm_delete(self, paciente_id, name):
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Text(f"¿Está seguro de eliminar al paciente '{name}'? Esta acción no se puede deshacer."),
            actions=[
    ...
    def check_duplicates_and_save(self, e):
        ...
        # 2. Check Duplicates (Only if Creating New, or always? User said "Al dar clic en Guardar (Nuevo Paciente)".
        # If updating, usually we skip this or check exclude self.
        if self.selected_paciente_id:
            # Editing - Skip duplicate check for now or implement "exclude self" logic?
            # Prompt implies "Nuevo Paciente" context for duplicates. Let's assume edit is safe/intentional.
            self.save_paciente()
            return
    ...
    def show_duplicate_warning(self, dupes):
        # dupes is list of tuples (id, nombre, edad, unidad, genero, dni, ...)

        list_dupes = ft.ListView(height=150)
        for d in dupes:
            # d index: 0=id, 1=name, 2=edad, 3=unidad, 4=genero... (based on SELECT *)
            # Actually select * return order might vary slightly if schema changed but usually stable.
            # Pacientes schema: id, nombreCompleto, edad, unidadEdad, genero, dni, telefono...
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Container(
                width=500,
                content=ft.Column([
                    ft.Text("Se encontraron pacientes similares en la base de datos:", color=ft.Colors.RED),
                    ft.Divider(),
                    list_dupes,
                    ft.Divider(),
    ...
    def save_paciente(self):
        try:
            ...
            print(f"Error: {ex}")
            self.page_ref.open(ft.SnackBar(ft.Text(f"Error al guardar: {ex}"), bgcolor=ft.Colors.RED))
