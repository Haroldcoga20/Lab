...
class PacientesView(ft.Column):
    ...
    def save_paciente(self, e):
        if not self.txt_nombre.value or not self.txt_edad.value:
            self.page.snack_bar = ft.SnackBar(ft.Text("Nombre y Edad son obligatorios"), bgcolor=ft.Colors.RED)
        ...
        try:
            ...
            self.page.snack_bar = ft.SnackBar(ft.Text("Paciente guardado"), bgcolor=ft.Colors.GREEN)
            self.page.snack_bar.open = True
            self.page.update()
