...
class CrearOrdenView(ft.Column):
    ...
    def save_orden(self, e):
        if not self.selected_paciente_id:
            self.page.snack_bar = ft.SnackBar(ft.Text("Seleccione un Paciente"), bgcolor=ft.Colors.RED)
            self.page.snack_bar.open = True
            self.page.update()
            return

        if not self.selected_items:
            self.page.snack_bar = ft.SnackBar(ft.Text("Agregue al menos un examen"), bgcolor=ft.Colors.RED)
            self.page.snack_bar.open = True
            self.page.update()
            return
        ...
        try:
            ...
            self.page.snack_bar = ft.SnackBar(ft.Text(f"Orden #{orden_id} Creada Exitosamente"), bgcolor=ft.Colors.GREEN)
            self.page.snack_bar.open = True
