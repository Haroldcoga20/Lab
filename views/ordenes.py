...
class OrdenesView(ft.Column):
    ...
    def load_ordenes(self):
        ...
        try:
            ...
            ordenes = db.get_ordenes_paginated(self.limit, self.offset, search, medico_id, estado)
            ...
            if not ordenes:
                ...
                for o in ordenes:
                    ...
                    f_str = fecha.strftime("%d/%m/%Y %H:%M") if fecha else ""
                        ...
                        ft.Container(
                            content=ft.Row([
                                ...
                                ft.Column([
                                    ...
                                    ft.Text(f"{f_str} | Estado: {estado_orden}", size=12, color=status_color, weight=ft.FontWeight.BOLD)
                                ], expand=True),
                                ft.Row([
                                    ft.IconButton(ft.Icons.PRINT, icon_color=ft.Colors.BLUE, on_click=lambda e, x=oid: self.open_config_dialog(x)),
                                    ft.IconButton(ft.Icons.DELETE, icon_color=ft.Colors.RED, on_click=lambda e, x=oid: self.confirm_delete(x))
                                ])
                            ]),
            ...
            # Pagination
            total = db.count_ordenes(search, medico_id, estado)
            self.btn_prev.disabled = (self.offset == 0)
            self.btn_next.disabled = (self.offset + self.limit >= total)
