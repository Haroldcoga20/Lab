...
def generar_pdf_orden(orden_id, config_list):
    ...
    for group_config in config_list:
        ...
        if group_config['page_break']:
             dibujar_firma() # Firma antes de saltar
             c.showPage()
             y = init_page()

        # Space Calculation (Tetris)
        # Espacio needed for Title + Method + Items
        espacio_needed = 40 + (len(items) * 15)
        # Limite inferior includes space for footer signature
        ...
        for item in items:
            ...
            try:
                ...
                # Only format if it doesn't lose data?
                # User requirement: "Todo resultado numérico debe imprimirse formateado con 2 decimales"
                val_str = "{:.2f}".format(float_val)
            except ValueError:
                pass # Not a number, keep as is (Text)
            ...
            c.drawString(margen_x + (ancho_util * 0.4), y, val_str)
            c.setFont("Courier", 10)
    ...
    # --- FIRMA FINAL ---
    # Asegurar firma en la última hoja
    dibujar_firma()
