...
def generar_pdf_orden(orden_id, config_list, include_signature=True):
    ...
    def dibujar_marca_agua():
        # CASO B: Si NO es Cespedes, dibujamos marca de agua
    ...
    def dibujar_barra_titulos(y_actual):
        ...
        # Centering adjustment: (0.4 + some width) -> drawCentredString at x + width/2
        # Or just adjusting X coordinate as requested. 0.4 starts at 40%.
        # Let's move it slightly to 0.38? Or maybe user meant the content?
        # User said "Result text appearing visually centered in its column".
        # Header is at 0.4. Content is at 0.4.
        # Let's leave header at 0.4 for now. Content drawing below uses same X.
    ...
    for group_config in config_list:
        ...
        for item in items:
            ...
            # Adjusted X for Result: Visual centering?
            # 0.4 is start. Maybe move slightly right to center under header?
            # Header "RESULTADO" is at 0.4. Text is 9 chars.
            # If value is "43.00" (5 chars).
            # Courier is fixed width.
            # Let's align it with the header X + small offset?
            # Header X: margen_x + (ancho_util * 0.4)
            # Let's add +15 points?
