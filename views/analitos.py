...
class AnalitosView(ft.Column):
    def __init__(self, page: ft.Page):
        ...
        # Restored Visual Reference Field (Phase 9)
        self.txt_visual = ft.TextField(label="Referencia Visual (Texto General)", expand=True)
        ...
        self.lbl_calc_note = ft.Text("Nota: Use abreviaturas en corchetes, ej: [COL] - [HDL]", size=12, color=ft.Colors.GREY, visible=False)
        self.txt_abreviatura = ft.TextField(label="Abreviatura", width=150)
        ...
        form_row1 = ft.Row([self.txt_nombre, self.txt_abreviatura, self.txt_unidad])
        form_row2 = ft.Row([self.dd_categoria, self.txt_metodo, self.txt_muestra, self.dd_tipo_dato])
        form_row3 = ft.Row([self.txt_subtitulo, self.txt_valor_defecto])
        form_row4 = ft.Row([self.chk_calculado, self.txt_formula])

        # Added txt_visual to form
        form_row5 = ft.Row([self.txt_visual])
        ...
        self.controls = [
            ...
            ft.Container(
                content=ft.Column([form_row1, form_row2, form_row3, form_row5, form_row4, self.lbl_calc_note, self.panel_opciones, actions_row]),
                padding=10,
                border=ft.border.all(1, ft.Colors.GREY_300),
                border_radius=5
            ),
            ft.Divider(),
            ft.Row([ft.Text("Lista de Analitos", size=16), self.txt_search], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
    ...
    def load_data(self):
        ...
        try:
            ...
            self.filter_analitos(None) # Initial render
        except Exception as e:
            print(f"Error loading analitos: {e}")

    def filter_analitos(self, e):
        search_term = self.txt_search.value.lower() if self.txt_search.value else ""
        self.table.rows.clear()

        for analito in self.analitos:
            if search_term and search_term not in analito.nombre.lower():
                continue

            abbr = getattr(analito, 'abreviatura', "") or ""
    ...
    def add_opcion(self, e):
        if not self.selected_analito_id:
            self.page_ref.open(ft.SnackBar(ft.Text("Primero guarde el analito para agregar opciones"), bgcolor=ft.Colors.RED))
            return
    ...
    def edit_analito(self, analito: Analito):
        ...
        self.txt_abreviatura.value = getattr(analito, 'abreviatura', "") or ""
        ...
        self.txt_subtitulo.value = getattr(analito, 'subtituloReporte', "") or ""
        self.txt_valor_defecto.value = getattr(analito, 'valorPorDefecto', "") or ""
        self.txt_visual.value = analito.referenciaVisual or "" # Restored
        ...
        self.lbl_calc_note.visible = analito.esCalculado
    ...
    def clear_form(self, e=None):
        ...
        self.lbl_calc_note.visible = False
    ...
    def save_analito(self, e):
        try:
            data = {
                ...
                'abreviatura': self.txt_abreviatura.value
            }
...
class RangosDialog:
    def __init__(self, analito_id, page):
        ...
        self.txt_panico_min = ft.TextField(label="Pánico Min", width=80, keyboard_type=ft.KeyboardType.NUMBER)
        self.txt_panico_max = ft.TextField(label="Pánico Max", width=80, keyboard_type=ft.KeyboardType.NUMBER)
        ...
        self.dialog = ft.AlertDialog(
            ...
            content=ft.Container(
                ...
                content=ft.Column([
                    ...
                    ft.Row([self.txt_val_min, self.txt_val_max, self.txt_panico_min, self.txt_panico_max, ft.ElevatedButton("Agregar", on_click=self.add_rango)]),
                    ft.Divider(),
            ...
            for r in rows:
                ...
                self.table_rangos.rows.append(
                    ft.DataRow(cells=[
                        ...
                        ft.DataCell(ft.Text(f"{rango.panicoMin or ''} - {rango.panicoMax or ''}")),
    ...
    def add_rango(self, e):
        try:
            data = {
                ...
                'panicoMin': self.txt_panico_min.value,
                'panicoMax': self.txt_panico_max.value
            }
            db.add_rango(data)
            self.load_rangos()
        except Exception as ex:
