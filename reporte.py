...
def generar_pdf_orden(orden_id, incluir_firma=True):
    ...
    # --- LÓGICA CONDICIONAL DE MÉDICO ---
    # Normalizamos el nombre para comparar sin problemas de mayúsculas
    ...
    def dibujar_marca_agua():
        # Solo dibujamos marca de agua si NO es el Dr. Cespedes (o si tú quieres que también salga, avísame)
        # Asumiré que para él quieres el reporte limpio. Si no, borra el 'if not ES_DR_CESPEDES:'
    ...
    def dibujar_membrete(y_actual):
        # SELECCIÓN DE IMAGEN SEGÚN MÉDICO
        if ES_DR_CESPEDES:
            # Caso Especial: Dr. Hugo Cespedes
            # Busca una imagen específica para él o usa una genérica limpia
            ruta_membrete = os.path.join(os.getcwd(), "assets", "membrete_cespedes.png")
        else:
            # Caso Estándar: Divino Niño
        ...
        if os.path.exists(ruta_membrete):
            ...
            # Si no hay imagen para el Dr. Cespedes, simplemente dejamos el espacio en blanco
            # o escribimos un texto simple si prefieres.
            if ES_DR_CESPEDES:
                 # Opcional: Escribir algo simple o dejar vacío
    ...
    if incluir_firma:
        ...
        if y < y_firma + 60: 
            c.showPage()
            dibujar_marca_agua() # Marca en nueva hoja de firma
            y_firma = margen_y_bottom + 40 
        
        x_firma_centro = w - margen_x - 100
        
        # FIRMA NORMAL (Por defecto)
        ruta_firma = os.path.join(os.getcwd(), "assets", "firma.png")
        
        # LÓGICA ESPECIAL: Si es el Dr. Cespedes, podríamos querer NO poner firma
        # o poner una firma diferente si él te la da.
        # Por ahora asumimos que él firma a mano o no lleva tu firma digital.
        if ES_DR_CESPEDES:
            # Si es para él, quizás no quieras poner TU firma digital
            pass 
        else:
            # Para los demás, firma normal
