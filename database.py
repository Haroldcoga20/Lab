...
class DatabaseManager:
    ...
    def get_ordenes_pendientes_filtradas(self, search_term=None, medico_id=None):
        # Specific for Resultados view (defaults to showing incomplete, but user wants filters)
        # Actually user asked for Status filter too in Results view?
        # Requirement: "Dropdown: Estado (Todos, Pendiente, Completado)" for both Views.
        # So we can reuse `get_ordenes_filtradas` but we might want different defaults?
        # Let's reuse `get_ordenes_filtradas` but maybe limit default view if no filters?
        # The logic in View will handle calling this.
        return self.get_ordenes_filtradas(search_term, medico_id, estado=None) # Allow all states if filtered, or UI handles it.
