...
class DatabaseManager:
    ...
    def get_resultados_grouped(self, orden_id):
        """
        Returns results grouped by Profile (Hierarchy) or Category (Single items).
        Structure:
        [
            {
                'type': 'Perfil',
                'title': 'PERFIL LIPÍDICO',
                'items': [ {id, analitoName, valor, estado, unidad, tipoDato, ...} ]
            },
            {
                'type': 'Categoria',
                'title': 'QUÍMICA',
        ...
        # We need to distinguish if the result row comes from a "Real Profile" or the "Default Profile" (Single items).
        # We also need the Profile Name and Analyte Category.
        ...
        query = """
            ...
            ORDER BY p.nombre, a.categoria, a.nombre
        """
        cursor.execute(query, (orden_id,))
        rows = cursor.fetchall()

        # Grouping Logic in Python
        groups = {} # Key: (Type, Title) -> List of items

        for row in rows:
            # Unpack
            rid, aid, aname, val, est, unit, dtype, cat, pid, pname = row

            item_data = {
                'id': rid, 'analitoId': aid, 'nombre': aname, 'valor': val,
                'estado': est, 'unidad': unit, 'tipoDato': dtype
            }

            if pid == default_profile_id:
                # Group by Category (Single Item)
        ...
        # Sort: Profiles first, then Categories
    ...
    def update_resultado_batch(self, updates):
        """
        updates: list of {'id': int, 'valor': str}
        """
        conn = self.get_connection()
        if not conn: return
        cursor = conn.cursor()
    ...
    def get_smart_reference(self, analito_id, p_genero, p_edad, p_unidad):
        ...
        for row in rows:
            # Index: 2=genero, 3=min, 4=max, 5=valMin, 6=valMax, 7=visual, 8=unidad, 9=interpretacion
            r_genero = row[2]
            r_min = row[3]
            r_max = row[4]
            r_val_min = row[5]
            ...
            r_visual = row[7]
            r_unidad = row[8]
            r_interp = row[9] # e.g. "Niños", "Adultos"
