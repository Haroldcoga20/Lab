...
class DatabaseManager:
    ...
    def get_all_analitos(self):
        ...
        if not conn: return []
        cursor = conn.cursor()
        # Assumes columns exist. If not, script must have added them.
        # Using * is risky if schema changes, but per instructions we adapt to new columns.
    ...
    def upsert_analito(self, data):
        ...
        # We need to know if columns exist? The prompt says "Asume que las tablas ... ya tienen las columnas necesarias".
        # So I will add them to query.
    ...
    def get_resultados_grouped(self, orden_id):
        ...
        # Fetch metadata: subtituloReporte (index 11), valorPorDefecto (index 12)
        # Note: indices depend on fetchall order.
        query = """
            SELECT
        ...
        for row in rows:
            rid, aid, aname, val, est, unit, dtype, cat, pid, pname, metodo, subtitulo, def_val = row

            # Fetch options if type is 'Opciones'
            opciones_list = []
            if dtype == 'Opciones':
                # Optimization: Could fetch all options for all involved analytes in one query?
                # For now, simplistic loop is cleaner code-wise unless performance hits.
                # 'get_opciones_analito' does SELECT .. FROM OpcionesAnalito WHERE analitoId=?
                opts = self.get_opciones_analito(aid) # [(id, aid, val, isDefault), ...]
                # Format for UI: [{'text': val, 'default': bool}]
                for o in opts:
